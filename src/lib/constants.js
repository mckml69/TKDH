...
export const TEMPLATES = {
  ...
  equipment: { key: "equipment", label: "Equipment Checks", short: "Equipment", icon: Wrench, mode: "recurring", accent: "#8A6D1F", assetEligible: true, roomEligible: true, contractorEligible: true,
    presets: ["Lift service", "Lift LOLER thorough examination", "PAT testing", "EICR (electrical installation)", "Kitchen equipment service", "Fire alarm system service", "Gas safety check", "AC filter clean", "AC service", "Ladder / access equipment check", "Cot / temporary bed check"] },
...
export const PRESET_DEFAULT_FREQUENCY = {
  ...
  "Lift service": 30, "Lift LOLER thorough examination": 182, "PAT testing": 365, "EICR (electrical installation)": 1825, "Kitchen equipment service": 365, "Fire alarm system service": 182, "Gas safety check": 365, "AC filter clean": 91, "AC service": 365, "Ladder / access equipment check": 30, "Cot / temporary bed check": 30,
...
export const ASSET_TYPES = [
  ...
  { key: "cot_temp_bed", label: "Cot / Temporary Bed", category: "equipment", prefix: "CTB", eligibleFor: ["equipment"] },
...
export const REQUIREMENTS = [
  ...
  { id: "water-temp", category: "legionella", matchCategories: ["legionella", "legionella_temp_check"], matchMode: "preset", matchValues: ["Outlet temperature check", "Legionella Water Temperature Check"], title: "Water Temperature Monitoring",
  ...
  { id: "cot-temp-bed", category: "equipment", matchMode: "preset", matchValues: ["Cot / temporary bed check"], title: "Cot / Temporary Bed Check",
    basis: ["risk"], sourceConfidence: "confirmed",
    whatToKeep: "A pre-use check before each cot or temporary/folding bed is issued to a guest, plus a periodic detailed inspection at an interval you set and document yourself.",
    why: "Cots and folding beds have known failure points — collapsing hinges, an unlocked side rail, a missing safety catch — that cause real injuries, particularly to children. BS EN 716 sets the safety standard the equipment itself has to meet, but doesn't set an ongoing inspection interval for a hotel using one.",
    frequency: "No fixed interval anywhere in law or official guidance. The general work-equipment duty (PUWER 1998) requires equipment to be maintained in an efficient state — the pre-use check before each guest use is the real control; the periodic interval is entirely your own hotel policy, same as the ladder check above.",
    evidence: "Inspection record noting date, inspector, and condition; pre-use checks don't need to be individually recorded.",
    retention: "Keep current inspection history.",
    legislation: "Provision and Use of Work Equipment Regulations (PUWER) 1998 (general duty, no fixed interval); BS EN 716 (product safety standard for cots/folding beds, not an inspection-interval regulation)." },
  { id: "window-restrictor", category: "window_restriction", matchCategories: ["window_restriction", "window_restriction_check"], matchMode: "preset", matchValues: ["Window restrictor check", "Window Restriction Check"], title: "Window Restrictor Check",
